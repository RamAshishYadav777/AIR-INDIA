...
const Chatbot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const userId = user?.id; // Using .id as per dashboard usage
      const { data: response } = await api.post("/chat", { message: input, userId });

      const botMsg: Message = { role: "bot", text: response.data.reply };
