...
import { addFlight } from "../../../hooks/redux/slices/flightSlice";
...
const AddFlight: React.FC = () => {
  ...
  const onSubmit = async (data: FlightForm) => {
    ...
    try {
      ...
      // Dispatch addFlight thunk
      const result = await dispatch(addFlight(flightData));

      if (addFlight.fulfilled.match(result)) {
        toast.success("✈️ Flight added successfully!");
        setTimeout(() => navigate("/dashboard/admin"), 1300);
        reset();
      } else {
        throw new Error(result.error?.message || "Failed to add flight");
      }
    } catch (err: any) {
