...
import { loadUser, logoutUser } from "../../../hooks/redux/slices/authSlice";
...
const UserDashboard: React.FC = () => {
  ...
  const userId = user?.id; // Assuming user.id corresponds to what API expects (or user._id if mongo)
  // If your authSlice.ts defines id as string, use that.

  const { data: bookings = [], isLoading: fetchingBookings } = useUserBookings(userId);

  const [loading, setLoading] = useState(true);
  const [tabIndex, setTabIndex] = useState(0);
  const [fullNameState, setFullNameState] = useState(""); // renamed to avoid conflict if any
  ...
  useEffect(() => {
    if (!user) {
      ...
      setFullNameState(user.fullName || user.username || "");
      setLoading(false);
    }
  }, [user, dispatch]);

  useEffect(() => {
    // If auth finishes and no user, redirect
    if (!authLoading && !user) {
      navigate("/login");
    }
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      await api.put('/auth/profile', {
        name: fullNameState.trim(),
        password: password || undefined
      });
      toast.success("Profile updated successfully!");
      setPassword("");
      dispatch(loadUser()); // Reload user to update redux state
    } catch (err: any) {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      // Invalidate query to refetch bookings
                    ...
                    <TableContainer component={Paper} elevation={0}>
                      <Table>
                        <TableHead sx={{ bgcolor: "#f5f5f5" }}>
                          <TableRow>
                            {["Flight", "From → To", "Date & Time", "Passenger", "Seat", "Status", "Check-in", "Action"].map((h) => (
                          ...
                          {bookings.map((b) => {
                            ...
                            return (
                              <TableRow key={b.id} hover>
                                <TableCell>
                                  <Box>
                                    <Typography fontWeight={900} color="error" sx={{ fontFamily: 'monospace', letterSpacing: 1 }}>{b.pnr || "------"}</Typography>
                                    <Typography variant="body2" fontWeight={700} sx={{ color: '#1a1a1a' }}>{b.flights?.flight_number}</Typography>
                                    <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.65rem' }}>Ref: {b.booking_id?.substring(0, 10)}</Typography>
                                  </Box>
                                </TableCell>
                                ...
                                <TableCell>
                                  {b.checked_in ? (
                                    ...
                                    <Button
                                      ...
                                      onClick={() => navigate("/checkin", { state: { bookingId: b.id } })}
                                    >
                                      Proceed
                                    </Button>
                                  )}
                        ...
                        return (
                          <Card key={b.id} sx={{ p: 2, borderRadius: 3, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
                            ...
                            <Box mb={2}>
                              {b.checked_in ? (
                                <Chip
                                  ...
                                  size="small"
                                  color="success"
                                  sx={{ fontWeight: "bold", width: '100%', borderRadius: 1.5 }}
                                />
                              ) : (
                                <Button
                                  variant="outlined"
                            ...
                            <Stack spacing={0.5} mb={2}>
                              <Typography variant="body2"><strong>PNR NO:</strong> <span style={{ color: '#d32f2f', fontWeight: 900, fontFamily: 'monospace' }}>{b.pnr || "------"}</span></Typography>
                              ...
                              <Typography variant="caption" color="text.secondary">Booking Reference: {b.booking_id}</Typography>
                            </Stack>
